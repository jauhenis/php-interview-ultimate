...
- **`JOIN ... ON`**: Defines how tables are linked together. It is processed _during_ the join operation.
- **`WHERE`**: Filters the resulting rows _after_ the join has been logically formed.
...
- **ANY (or SOME):** Returns true if the comparison is true for _at least one_ value in the subquery result.
- **ALL:** Returns true only if the comparison is true for _all_ values in the subquery result.
